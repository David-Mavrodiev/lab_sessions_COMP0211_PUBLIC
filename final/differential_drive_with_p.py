...
def run(goal_state=None):
    ...
    Qcoeff = np.array([250, 250, 280.0]) # Best so far
    Rcoeff = [10, 5]
    ...
    return base_pos_all, base_bearing_all
...
if __name__ == '__main__':
    base_pos_all, base_bearing_all = run()
    base_pos_all = np.array(base_pos_all)
    plt.figure()
    plt.plot(base_pos_all[:, 0], base_pos_all[:, 1], label='True Path')
    plt.legend()
    plt.grid()
    # plt.savefig("/Users/joefarah/Desktop/Figures/E&C_Final/Task_2/path._with_P.png")
    plt.show()
    ...
    plt.legend()
    plt.grid()
    # plt.savefig("/Users/joefarah/Desktop/Figures/E&C_Final/Task_3/bearing_with_P.png")
    plt.show()
