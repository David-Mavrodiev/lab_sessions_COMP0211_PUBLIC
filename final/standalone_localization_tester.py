...
plt.axis('equal')
plt.grid(True)
# plt.savefig('/Users/joefarah/Desktop/unicycle_robot_localization.png')
plt.show()
...
for s in range(3):
    ...
    sigma_avg = np.mean(two_sigma)
    plt.plot(estimation_error[:, s])
    plt.plot(two_sigma, linestyle='dashed', color='red')
    plt.plot(-two_sigma, linestyle='dashed', color='red')
    if s < 2:
        plt.axhline(y=0.1, color='lime', label="Upper Threshold (0.1)")
        plt.axhline(y=-0.1, color='lime', label="Lower Threshold (-0.1)")
    plt.title(state_name[s])
    # plt.savefig(f'/Users/joefarah/Desktop/unicycle_robot_localization_error_{s}.png')
    plt.show()
