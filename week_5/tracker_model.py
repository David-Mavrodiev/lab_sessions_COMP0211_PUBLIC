...
class TrackerModel:
    def __init__(self, A_cont, B_cont, C_cont, Q, R, N, q, m, n):
        self.A_cont = A_cont
        self.B_cont = B_cont
        self.C_cont = C_cont
        self.Q = Q
        ...
        self.A = np.block([
            ...
            [np.zeros((self.m, self.n)), np.eye(self.m)]
        ])
        ...
        self.C = np.block([
            [self.C_cont, np.zeros((self.q, self.m))]
        ])

        self.n = self.A.shape[0]

        self.u_cur = np.zeros((self.m,))
    ...
    def computesolution(self,x_ref, x_cur, u_cur, H, F_tra):
        ...
        x0 = np.zeros((u_cur.shape[0]*self.N, )) # Assuming u_cur has the correct dimension
