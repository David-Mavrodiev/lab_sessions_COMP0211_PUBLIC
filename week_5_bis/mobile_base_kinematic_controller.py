...
def main():
    ...
    # Plot trajectory
    axs[0].plot(base_pos_all[:, 0], base_pos_all[:, 1], label='Robot Trajectory')
    axs[0].scatter(base_pos_all[0, 0], base_pos_all[0, 1], color='g', label='Start')
    axs[0].scatter(base_pos_all[-1, 0], base_pos_all[-1, 1], color='r', label='End')
    ...
    # Plot orientation
    base_bearing_all = [quaternion2bearing(q[3], q[0], q[1], q[2]) for q in base_ori_all]
    axs[1].plot(base_bearing_all, label='Robot Orientation')
    axs[1].set_xlabel('Time Steps')
    axs[1].set_ylabel('Theta (radians)')
    axs[1].set_title('Robot Orientation')
    axs[1].legend()
    axs[1].grid(True)
