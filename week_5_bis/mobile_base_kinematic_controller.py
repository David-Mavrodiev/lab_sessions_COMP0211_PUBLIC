...
def main():
    ...
    # Position plot for x, y
    plt.subplot(2, 1, 1)
    plt.plot([pos[0] for pos in base_pos_all], [pos[1] for pos in base_pos_all], label='Estimated Position')
    plt.title('Estimated Position Tracking')
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.legend()

    # Orientation plot for theta
    plt.subplot(2, 1, 2)
    plt.plot([bearing for bearing in base_ori_all], label='Estimated Orientation')
    plt.title('Estimated Orientation Tracking')
